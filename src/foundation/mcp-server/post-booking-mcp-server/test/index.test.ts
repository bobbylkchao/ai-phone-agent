...
import { initPostBookingMcpServer } from '../index'
...
describe('post-booking MCP HTTP route', () => {
  ...
  it('registers tools and serves one stateless request per call', async () => {
    initPostBookingMcpServer(app, 4000)
    ...
    await routes.get('/post-booking-mcp')?.(req, res as unknown as Response)
  ...
  it('returns 500 when the MCP server cannot connect', async () => {
    ...
    const res = new FakeResponse()
    await routes.get('/post-booking-mcp')?.(
      { body: {} } as Request,
  ...
  it('absorbs initialization errors without registering a route', () => {
    ...
    expect(() => initPostBookingMcpServer(app, 4000)).not.toThrow()
