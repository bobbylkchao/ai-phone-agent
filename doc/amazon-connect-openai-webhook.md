...
- **MCP:** `VoiceAgentDefinition.getMcpServers` attaches public HTTPS MCP endpoints to the Realtime call. The hotel example points `HOTEL_BOOKING_MCP_SERVER_URL` at its `/hotel-booking-mcp` stub and allowlists `search-hotel`; replace the URL with a compatible production MCP provider when ready.
...
- **MCP silence**: Remote MCP tools run inside OpenAI, and the response that issued the call can finish while the tool is still running, so the caller waits in silence with no turn in flight. `websocket/mcp-response-continuation.ts` waits for `conversation.item.done` with an `mcp_call` item — the result landing in the conversation — then sends `response.create`, deferring if a response is still generating. Capped at three consecutive tries per call. Asking any earlier makes the model say the search is still running.
